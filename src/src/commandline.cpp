...
std::optional<int> CreatePortableCommand::runEarly()
{
  ...
  const auto launcher = portable_launcher_script::create(instanceDir);
  if (launcher.status == portable_launcher_script::Status::Failed) {
    std::cerr << "Error: " << launcher.error.toStdString() << "\n";
    return 1;
  }

  std::cout << "Portable instance created: " << instanceDir.toStdString() << "\n";
  std::cout << "  Launcher script: " << launcher.path.toStdString() << "\n";

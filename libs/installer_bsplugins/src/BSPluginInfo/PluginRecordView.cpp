...
void PluginRecordView::on_pickRecordView_customContextMenuRequested(const QPoint& pos)
{
  ...
  using Item = TESData::FileEntry::TreeItem;
  const auto items =
      std::ranges::transform_view(selectedRows, [](const QModelIndex& index) {
        return index.data(Qt::UserRole).value<const Item*>();
      });

  if (items.empty() || !std::ranges::all_of(items, [](const Item* item) {
  ...
  ignoreRecord->setChecked(std::ranges::all_of(items, [](const Item* item) {

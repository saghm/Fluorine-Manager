...
#else
    Dl_info info;
    if (dladdr((void*)&getPluginFolder, &info) == 0 || info.dli_fname == nullptr) {
        return {};
    }
...
bool ProxyPython::init(IOrganizer* moInfo)
{
...
#ifdef _WIN32
    const auto pluginDataRoot =
        fs::exists(pluginFolder / "plugin_python") ? (pluginFolder / "plugin_python")
                                                   : pluginFolder;
#else
    // Linux portable installs can contain a legacy Windows "plugin_python"
    // payload (pythoncore.zip/.pyd) that is incompatible with our bundled
    // Linux runtime. Always use the plugin root on Linux.
    const auto pluginDataRoot = pluginFolder;
#endif
...
#else
        m_LoadFailure = FailureType::DLL_NOT_FOUND;
        log::error("failed to resolve Python proxy directory: {}",
                   std::strerror(errno));
#endif
        return false;
    }
    ...
    if (m_Runner) {
        const auto libpath = pluginDataRoot / "libs";
#ifdef _WIN32
        ...
        if (envPy && envPy[0] != '\0') {
            pythonDir = fs::path(envPy);
        } else {
            pythonDir = fs::path(QCoreApplication::applicationDirPath().toStdWString()) / "python";
        }
        paths.emplace_back(pythonDir / "site-packages");
        paths.emplace_back(pythonDir);
...
#endif
#ifdef MO2_PYTHON_PLATLIB_DIR
        if (std::strlen(MO2_PYTHON_PLATLIB_DIR) > 0 &&
            std::strcmp(MO2_PYTHON_PLATLIB_DIR, MO2_PYTHON_PURELIB_DIR) != 0) {
            paths.emplace_back(MO2_PYTHON_PLATLIB_DIR);
        }
#endif
        m_Runner->initialize(paths);
#endif
